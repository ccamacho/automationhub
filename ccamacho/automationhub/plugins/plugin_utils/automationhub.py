...
class AutomationHub(object):
    ...
    def get_namespace(self):
        ...
        searches for the existance of a namespace given a repo
        :return: str text
        """
        page_size = 1       
        ...
        try:
            what= 'namespaces'
            url = _urljoin(api.api_server,'api', api.available_api_versions['v1'], what, "?name="+ self.options.repo.split("/")[0]+"&page_size="+str(page_size))
    ...
    def install(self, book_id):
        ...
        if len(self.options.repo) == 0 and role_file is None:
        ...
        if role_file:
            ...
            for rname in self.options.repo:
        ...
        for role in roles_left:
            ...
            if role_file and self.options.repo and role.name not in self.options.repo:
    ...
    def search(self):
        ...
        tokken = GalaxyToken(token=self.options.token)
        print(self.options.repo)
        api = GalaxyAPI(self.galaxy, "test", self.options.api_server, token=tokken)

        if len(self.options.repo):
            terms = []
            for i in range(len(self.options.repo)):
                terms.append(self.options.repo.pop())
            search = '+'.join(terms[::-1])
        ...
        response = api.get_collection_metadata("kubeinit","kubeinit")
        print(response)
        response = api.search_roles(search, page_size=page_size, author='kubeinit')
        print(response)
